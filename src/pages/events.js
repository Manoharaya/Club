  ...
  function renderGrid() {
                ...
                <div class="event-card-body">
                  <div>
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; gap: 1rem;">
                      ...
                      <span class="event-status-badge ${spots <= 5 ? "urgent" : ""}">
                        ${isSoldOut ? "Sold Out" : `Only ${spots} spots left!`}
                      </span>
                    </div>
                    <h3 class="event-title">${ev.title}</h3>
                    <p class="event-desc">${ev.description}</p>
                    <p class="event-meta">📅 ${ev.date} &bull; ⏱ ${ev.time}</p>
                    <p class="event-meta" style="margin-top: 0.25rem;">👤 Instructor: ${ev.instructor}</p>
                  </div>
    ...
    regForm.addEventListener("submit", (e) => {
      ...
      if (activeEventId && window.eventSpotsLeft[activeEventId] > 0) {
        ...
        showNotification(
          "Registered",
          "Pre-registration confirmed! Check email inbox.",
          "success",
        );
        closeModal();
